...
int exec_remote_cmd_loop(char *server_ip, int port) {
    int sock;
    int rc = OK;
    char *send_buf = malloc(RDSH_COMM_BUFF_SZ);
    ...
    if (sock < 0) {
        rc = ERR_RDSH_CLIENT;
        return client_cleanup(sock, send_buf, recv_buf, rc);
    }
    
    while (1) {
        printf("remote dsh> ");
        fflush(stdout);
        if (!fgets(send_buf, RDSH_COMM_BUFF_SZ, stdin)) break;
        ...
        // Send command (include the null terminator)
        ...
        // Loop to receive until we see the EOF marker (RDSH_EOF_CHAR)
        while (1) {
            int bytes = recv(sock, recv_buf, RDSH_COMM_BUFF_SZ, 0);
            if (bytes <= 0) {
                fprintf(stderr, "Client: recv error or connection closed\n");
                goto cleanup;
            }
            // Check if last byte is the EOF marker
            if ((unsigned char)recv_buf[bytes-1] == RDSH_EOF_CHAR) {
                // Replace EOF marker with null and print the chunk
                recv_buf[bytes-1] = '\0';
                printf("%s", recv_buf);
                break;
            } else {
                // Print the received chunk as-is
                printf("%.*s", bytes, recv_buf);
            }
        }
    }
    
cleanup:
    rc = client_cleanup(sock, send_buf, recv_buf, OK);
    return rc;
}
