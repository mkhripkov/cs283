...
int boot_server(char *ifaces, int port) {
    ...
    setsockopt(svr_sock, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));

    server.sin_family = AF_INET;
    server.sin_addr.s_addr = inet_addr(ifaces);
...
int process_cli_requests(int svr_socket) {
    while (1) {
        int cli_sock = accept(svr_socket, NULL, NULL);
        if (cli_sock < 0) {
            perror("accept");
            return ERR_RDSH_COMMUNICATION;
        }
...
int exec_client_requests(int cli_socket) {
    char *recv_buf = malloc(RDSH_COMM_BUFF_SZ);
    if (!recv_buf)
        return ERR_MEMORY;
    while (1) {
        int total = 0;
        while (1) {
            int r = recv(cli_socket, recv_buf + total, 1, 0);
            if (r <= 0) {
                free(recv_buf);
                return ERR_RDSH_COMMUNICATION;
            }
        ...
        if (strchr(recv_buf, PIPE_CHAR) != NULL) {
            command_list_t clist;
            clist.num = 0;
            int rc = build_cmd_list(recv_buf, &clist);
            if (rc == OK) {
                int pipeline_status = rsh_execute_pipeline(cli_socket, &clist);
                /* Optionally report pipeline_status if needed */
                free_cmd_list(&clist);
            } else {
                char error_msg[] = "Error parsing pipeline command\n";
        ...
        cmd_buff_t local_cmd;
        alloc_cmd_buff(&local_cmd);
        build_cmd_buff(recv_buf, &local_cmd);

        Built_In_Cmds bic = match_command(local_cmd.argv[0]);
        if (bic != BI_NOT_BI) {
            if (bic == BI_CMD_EXIT) {
                free_cmd_buff(&local_cmd);
                break;
            }
            if (bic == BI_CMD_CD) {
                exec_built_in_cmd(&local_cmd);
                send_message_eof(cli_socket);
            } else {
                pid_t pid = fork();
                if (pid == 0) {
            ...
            if (pid == 0) {
                dup2(cli_socket, STDOUT_FILENO);
                dup2(cli_socket, STDERR_FILENO);
                execvp(local_cmd.argv[0], local_cmd.argv);
                _exit(errno);
            } else {
                waitpid(pid, NULL, 0);
                send_message_eof(cli_socket);
...
int send_message_string(int cli_socket, char *buff) {
    ...
    while (total_sent < send_len) {
        int sent = send(cli_socket, buff + total_sent, send_len - total_sent, 0);
        if (sent <= 0) {
            return ERR_RDSH_COMMUNICATION;
        }
        total_sent += sent;
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    return WEXITSTATUS(statuses[n-1]);
}
